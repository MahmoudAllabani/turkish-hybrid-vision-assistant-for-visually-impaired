...
| Intent classification | Android edge | Compact LiteRT 1D-CNN plus deterministic fallback supports offline command handling. |
| OCR and spoken feedback | Android edge | Printed-text reading works without internet; OCR images are not uploaded. |
| Wearable image capture | Local Wi-Fi | ESP32/M5Stack camera communicates directly with the phone. |
...
The client first checks connectivity, bounds the image to 800 pixels, compresses it as JPEG at 70% quality, and sends multipart data. FastAPI authenticates requests through `X-API-Key`; the application key, Groq key, database URL, and database-ping token are supplied as environment variables. The service calls Groq's `meta-llama/llama-4-scout-17b-16e-instruct` with instructions that prioritize people, motion, hazards, obstacles, doors, stairs, vehicles, and spatial context.
...
> The scene-analysis flow sends an optimized image to Llama 4 Scout only after an explicit online request, then returns its contextual Turkish description for spoken feedback.
...
<p align="center">
  <img src="docs/images/05-render-backend-dashboard.jpeg" width="700" alt="Render backend dashboard">
  <br><br>
  <img src="docs/images/06-postgresql-telemetry-logs.jpeg" width="700" alt="PostgreSQL telemetry logs">
</p>
...
|---|---:|---|
| Supported intent classes | 9 | Focused, action-oriented command vocabulary. |
| Augmented Turkish examples | 10,700 | 8,560 training and 2,140 validation examples. |
| Model parameters | 265,757 | Optimized for edge deployment. |
| LiteRT model size | 104 KB | Minimal mobile footprint. |
| Intent accuracy | 99.66% | Closed-set, task-specific evaluation. |
| Offline OCR | ~843 ms | No external network round trip. |
| Cloud scene analysis | ~821 ms | Multimodal reasoning is offloaded to the cloud. |
...
| Compute target | Android device | Render-hosted FastAPI + Groq MLLM |

The accuracy result should be read in context: it reflects a narrow closed command set, not unrestricted conversational Turkish.
